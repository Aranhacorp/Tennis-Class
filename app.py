...
# 4. CSS E COMPONENTES FIXOS (Blindagem contra SyntaxError)
st.markdown("""
<style>
    .stApp {
    ...
    /* DESIGN CLEAN - SEM BALÕES VERDES */
    .clean-link {
        text-align: center;
    ...
    .clean-link:hover { transform: translateY(-5px); color: #4CAF50 !important; }
    .icon-text { font-size: 80px; margin-bottom: 5px; }
    .label-text { font-size: 22px; font-weight: bold; letter-spacing: 2px; }
...
with st.sidebar:
    ...
    unidades = {"PLAY TENNIS Ibirapuera": "R. Estado de Israel, 860", "TOP One Tennis": "Premium", "MELL Tennis": "Sul", "ARENA BTG Morumbi": "Morumbi"}
    for nome, local in unidades.items():
        st.markdown(f"📍 **{nome}**\n<div style='font-size:11px; color:#ccc;'>{local}</div>", unsafe_allow_html=True)
...
if st.session_state.pagina == "Home":
    ...
    if not st.session_state.pagamento_ativo:
        with st.form("reserva_form"):
            ...
            servico = st.selectbox("Serviço", ["Aulas particulares R$ 250/h", "Aulas em Grupo R$ 200/h", "Treinamento R$ 1.400/mes"])
            unid = st.selectbox("Unidade", list(unidades.keys()))
            c1, c2 = st.columns(2)
        ...
        st.info("Chave: aranha.corp@gmail.com")
        timer_box = st.empty()
        ...
        timer_box.warning(f"⏱️ Tempo restante: {m:02d}:{s:02d}")
        time.sleep(1)
        st.rerun()
    ...
    # PORTAL SEM BALÕES - APENAS ÍCONES E NOMES
    with col1:
    ...
    if not st.session_state.admin_autenticado:
        ...
        if st.button("Acessar"):
            if pwd == "aranha2026": st.session_state.admin_autenticado = True; st.rerun()
