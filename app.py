...
# 2. ESTILIZAÇÃO CSS (DESIGN DARK GLASS)
st.markdown("""
    <style>
    .stApp {
                ...
                if st.form_submit_button("CONFIRMAR RESERVA"):
                    if aluno:
                        ...
                        if not conflito.empty:
                            st.error(f"❌ Horário {horario} já reservado para esta data.")
                        else:
...
# ABA DE CADASTRO COM GOOGLE FORMS INTEGRADO [SOLICITADO]
elif menu == "Cadastro":
    st.markdown("<h2 style='text-align: center; color: white;'>Cadastro de Professor</h2>", unsafe_allow_html=True)
    
    # URL de Incorporação do seu Google Form
    # Certifique-se de usar a URL que termina em '/viewform?embedded=true'
    form_url = "https://docs.google.com/forms/d/e/1FAIpQLSfN-d-T_G2V_u_yN0_S_b8O_G2H_u_yN0_S_b/viewform?embedded=true"
    
    st.markdown(f"""
        <div style="display: flex; justify-content: center;">
            <iframe src="{form_url}" width="800" height="900" frameborder="0" marginheight="0" marginwidth="0" 
