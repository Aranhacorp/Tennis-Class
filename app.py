...
# 4. CSS GLOBAL E COMPONENTES FIXOS (Corrigindo erro de aspas triplas)
st.markdown("""
<style>
    .stApp {
...
# 5. MENU LATERAL E ACADEMIAS ATUALIZADAS [cite: 2026-01-28]
with st.sidebar:
...
if st.session_state.pagina == "Home":
    ...
    if not st.session_state.pagamento_ativo:
        with st.form("reserva_form"):
            ...
            # Corrigindo falha de gravação de Serviço e Unidade
            serv_list = ["Aula Individual (R$ 250/h)", "Aula em Grupo R$ 200/hora", "Aula Kids (R$ 200/h)", "Treinamento competitivo: R$ 1.400 / mês"]
            servico = st.selectbox("Serviço", serv_list)
        ...
        st.warning(f"⏱️ Expira em: {m:02d}:{s:02d}")
    ...
    * **Eventos:** Sob Consulta
    ...
    # Restaurando os links de cadastro (Corrigindo erro)
    with col1:
    ...
    if not st.session_state.admin_autenticado:
        if st.text_input("Chave Mestra", type="password") == "aranha2026":
