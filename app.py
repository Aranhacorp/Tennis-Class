...
# Certifique-se que o e-mail do Service Account está como Editor na planilha
...
# 4. DESIGN, ASSINATURA E WHATSAPP
st.markdown("""
<style>
    .stApp {
    ...
    /* Estilos para Assinatura e WhatsApp */
...
if st.session_state.pagina == "Home":
    ...
    if not st.session_state.pagamento_ativo:
        with st.form("reserva_form"):
            ...
            servico = st.selectbox("Escolha o Serviço", [
                ...
                "Aula Kids (R$ 200/hora)", "Treinamento Esportivo (R$ 1.200/mês)"
            ])
        ...
        st.markdown(f"### 💳 Pagamento PIX")
        st.write(f"**Aluno:** {st.session_state.reserva_temp['Aluno']}")
        st.code("aranha.corp@gmail.com.br", language="text")
        if st.button("CONFIRMAR AGENDAMENTO E SALVAR"):
            try:
                # GRAVAÇÃO NA PLANILHA GOOGLE SHEETS
                df_nova = pd.DataFrame([st.session_state.reserva_temp])
                conn.create(data=df_nova) # Envia os dados para a planilha
                
                st.success("Reserva salva com sucesso na TennisClass_DB!")
                st.balloons()
                st.session_state.pagamento_ativo = False
    ...
    st.markdown('<div class="translucent-balloon"><h2>🎾 Tabela de Preços</h2>'
                '<ul><li><b>Individual:</b> R$ 250/h</li>'
                '<li><b>Grupo/Kids:</b> R$ 200/h</li>'
                '<li><b>Treinamento:</b> R$ 1.200/mês</li></ul></div>', unsafe_allow_html=True)

elif st.session_state.pagina == "Cadastro":
    st.markdown('<div class="translucent-balloon"><h3>📝 Portal de Cadastros Oficiais</h3>', unsafe_allow_html=True)
    ...
    st.markdown('<a href="https://docs.google.com/forms/d/e/1FAIpQLScyM6VvP0n_M0zIe-W_XzVw_v_v_v/viewform" class="btn-cadastro">🏢 Cadastro de Academia</a>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="translucent-balloon"><h3>📞 Contato</h3>'
                '<p>📩 aranha.corp@gmail.com.br<br>📱 (11) 97142-5028</p></div>', unsafe_allow_html=True)

elif st.session_state.pagina == "Produtos":
    st.markdown('<div class="translucent-balloon"><h3>🎒 Loja</h3><p>Em breve novidades.</p></div>', unsafe_allow_html=True)
